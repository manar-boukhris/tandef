...
export default function BookingPetsPage() {
  const router = useRouter();
  const [hasPets, setHasPets] = useState(true);

  useEffect(() => {
    document.title = "TANDEF – Hast du Haustiere?";
    const draft = getDraft();
    if (typeof draft.hasPets === 'boolean') setHasPets(draft.hasPets);
  ...
  min-height:100vh;
        ...
        .choice-card{background:#fff;border:2px solid #ECE8F5;border-radius:18px;transition:.2s ease;cursor:pointer;}
        .choice-card:hover{border-color:#C9B8EC;}
        .choice-card.selected{border-color:var(--purple-600);box-shadow:0 15px 35px -20px rgba(76,29,149,.35);}
        .choice-card.selected .check-circle{background:var(--purple-600);border-color:var(--purple-600);}
        .check-circle{width:24px;height:24px;border-radius:9999px;border:2px solid #D6CFE6;display:flex;align-items:center;justify-content:center;transition:.15s ease;flex-shrink:0;}
        ...
        .progress-fill{background:var(--purple-600);border-radius:9999px;height:6px;width:78%;}
      ...
      <section className="relative max-w-5xl mx-auto px-6 pt-6 pb-24">
        <div className="flex items-center justify-between mb-8">
          <a href="/booking-recipient" className="flex items-center gap-2 font-semibold hover:opacity-70" style={{color: 'var(--purple-700)'}}>
        ...
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl md:text-4xl font-extrabold mb-8" style={{color: 'var(--ink)'}}>Hast du Haustiere?</h1>

          <div className="space-y-4 mb-6">
            <div onClick={() => setHasPets(true)} className={`choice-card p-6 flex items-center gap-5 ${hasPets ? 'selected' : ''}`}>
              <div className="icon-tile">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#5B21B6" strokeWidth="2"><circle cx="7" cy="8" r="1.6" /><circle cx="16" cy="8" r="1.6" /><path d="M12 12c-3 0-6.5 2-6.5 5.2 0 2 1.8 3 3.6 2.3.9-.3 1.8-.3 2.9-.3s2 0 2.9.3c1.8.7 3.6-.3 3.6-2.3C18.5 14 15 12 12 12z" /></svg>
              </div>
              <div className="flex-1"><p className="font-bold" style={{color: 'var(--ink)'}}>Ja ich liebe sie!</p></div>
              <span className="check-circle shrink-0">{hasPets && <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="M5 12l5 5 9-9" /></svg>}</span>
            </div>

            <div onClick={() => setHasPets(false)} className={`choice-card p-6 flex items-center gap-5 ${!hasPets ? 'selected' : ''}`}>
              <div className="icon-tile">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#5B21B6" strokeWidth="2"><circle cx="7" cy="8" r="1.6" /><circle cx="16" cy="8" r="1.6" /><path d="M12 12c-3 0-6.5 2-6.5 5.2 0 2 1.8 3 3.6 2.3.9-.3 1.8-.3 2.9-.3s2 0 2.9.3c1.8.7 3.6-.3 3.6-2.3C18.5 14 15 12 12 12z" /><path d="M3 3l18 18" stroke="#B8AFC9" /></svg>
              </div>
              <div className="flex-1"><p className="font-bold" style={{color: 'var(--ink)'}}>Nein</p></div>
              <span className="check-circle shrink-0">{!hasPets && <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="M5 12l5 5 9-9" /></svg>}</span>
            </div>
          </div>
          ...
          <div className="info-banner mb-8">
            ...
            <p className="text-sm" style={{color: 'var(--muted)'}}>Wir verlangen diese Information, weil einige der Reinigungskräfte Allergien haben.</p>
          </div>
